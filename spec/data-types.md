...
基本数据类型是虚拟机（或处理器）直接支持的数据类型。大部分虚拟机都支持整型（包括符号和无符号整型）、浮点型两种，XiaoXuan 也大致如此。
...
XiaoXuan 支持大部分 Rust 字符串格式化格式，可以参考《Programming Rust》 P337 "格式化值" 一节。
...
散列字符串的字面量是使用 `#` （井字符号）加上一个无需双引号的字符串组成。
