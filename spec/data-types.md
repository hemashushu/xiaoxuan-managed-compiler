...
在第一个单反斜杠后面换行，则第一行正文前缀的空白将会被截断（trim），正文其他行的前缀空白将会按照第一行的前缀空白数量自动截断。
...
在第一个三双引号后面换行，则正文部分的第一行（即上面例子的 02 行）的第一个非空白字符表示正文的开始，正文部分其他行的前缀空白字符会根据正文第一行的前缀空白个数**自动截除**（空白字符不足的话则只截取实际个数空白）。上面示例的字符串的值为 `世界\n🈳🈳你好`（`"hello\n🈳🈳world"`）。
