...
整数的字面量（literals，即直接写在源码里的数据，硬编码数据）如 `123`，`1000` 等，默认类型为 `整数`，也就是 `整数64`。
...
字符是用于显示的单个字母、数字或者汉字等，字符的数据类型是 `字符`（`Char`），字符字面量使用一对单引号包围一个字符，比如 `'a'`, `'8'`, `'中'` 等。
...
字符在运行环境内部使用 `自然数32`（`Natural32`） 来处理（即，在内存中的表示方式）。但字符类型不能跟 `自然数32` 或者其他整数类型直接相互转换，因为部分整数不是有效的 Unicode。

注意字符（及字符串）在不同情况下，使用不同的处理方式：

* 在运行环境的内存里，为了方便和简化字符索引，XiaoXuan 使用 `自然数32` 表示字符；
* 在网络传输以及存储设备上存储字符，使用 UTF-8 编码格式。内部的字符需要转换成 UTF-8 方可以通过网络传输或者在存储设备上读写；
* 在显示及打印字符时，则根据使用 Unicode 标准来处理，需要注意的是一个视觉上的字符（一般称为 `字素`、`Grapheme`）不一定等于内部的一个字符，视觉上一个字符决定了文本编辑时的选定及光标移动，具体见 [Unicode 标准](https://en.wikipedia.org/wiki/Unicode)。
...
字符串即一串字符，字符串类型为 `字符串`（`String`），字符串字面量使用双引号（"）包围一串字符，如 `"abc"`，`"你好"`。字符串字面量必须在一行之内写完，不能分成多行。
...
因为字符串常用于映射表的键，但字符串只是 `数组<字符>`（`Array<Char>`）的别称，缺乏计算散列值的特性，所以准备使用字符串作为映射表的键时，可以使用 `散列字符串`（`KeyString`） 转换普通字符串。

示例：

```js
让 map1 = 新建 映射表<KeyString, Int>()
让 key1 = 新建 KeyString("abc")
让 map2 = map1.添加(key1, 100)
让 value1 = map2[key1]  # value1 == 100
```

散列字符串的字面量是使用 `#"..."` 包围一个字符串组成。
...
<!-- 如果字符串里面含有诸如空格，标点符号等，则可以使用双引号包围起来，即在字符串字面量前后添加 `#` 美元符号。 -->
