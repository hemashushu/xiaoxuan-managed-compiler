...
`遍历`（`iterate`）语句用于遍历一个列表或者实现了 `序列` 特性的数据到一个指定的变量，相当于 Java 语言的 "for...each" 语句。
...
`遍历` 语句的作用类似一个递归调用的匿名函数，该函数有且只有一个参数，即 `到` 关键字后面的变量，遍历语句使用 `[x,...xs]` 模式从一个 "类列表" 获取第一个元素及余下的元素，然后让第一个元素作为实参调用该匿名函数，重复这个过程直到类列表的元素都被读取一遍。
...
* `遍历` 语句无返回值，如果想遍历一个列表且根据一定的关系返回值，可以使用 `映射`（`map`） 或者 `折叠`（`fold`） 函数。
* 在 `遍历` 语句主体（循环主体）里的 `返回`（`return`）关键字会直接退出当前语句所在的函数，而不是中断当次循环或者退出循环语句。
...
`设有 取自` 语句的作用类似一个递归调用的匿名函数，该函数有且只有一个参数，即 `设有` 关键字后面的变量，遍历语句使用 `[x,...xs]` 模式从一个 "类列表" 获取第一个元素及余下的元素，然后让第一个元素作为实参调用该匿名函数，重复这个过程直到类列表的元素都被读取一遍。
...
* 在 `设有 取自` 语句主体（循环主体）里的 `返回`（`return`）关键字会直接退出当前语句所在的函数，而不是中断当次循环或者退出循环语句。
...
`设有 让` 语句在作用上类似一个只有一个参数的递归调用的匿名函数。
...
语句的第一行的作用类似声明了一个只有一个参数的匿名函数，且该参数会被赋予初始值。接下来是一个普通的 `如果` 条件语句，而 `重复` 关键字相当于调用当前匿名函数自身。
...
* 在 `设有 让` 语句主体（循环主体）里的 `返回`（`return`）关键字会退出当前语句所在的函数，而不是中断当次循环或者退出循环语句。
...
因为 `设有 让` 循环语句相当于一个递归调用的匿名函数，所以也是可以有返回值的，一般把返回值写在 `如果` 语句的 `否则` 位置。
...
上例代码实现从整数 1 累加到 100，然后将总和值（即变量 `s` 的值）通过 `设有` 语句返回。
...
`重复次数` 语句实际上是 `设有 取自` 语句的语法糖，上面的语句相当于：
