...
`设有 让`（`for let`）是一种类似 Java 的 "for(;;)" 语句的循环语句（由：一个变量及初始值、一个条件表达式、让变量变化的语句以及循环主体共 4 个部分组成的循环语句）。
...
* `让 i = 0` 是循环的变量名称及初始值，条件循环语句只能设置一个变量（当然变量可以是任何数据类型，包括元组，所以等同于支持多个变量）
...
其中 01 行是定义匿名函数，06 行是调用函数自己（相当于 `loop` 关键字），10 行是初始值及开始执行。
...
* 条件循环语句不支持 `continue` 和 `break` 等关键字。
* 其中的 `如果` 语句是标准的条件语句，如非必要，一般不需要书写 `否则` 部分。
