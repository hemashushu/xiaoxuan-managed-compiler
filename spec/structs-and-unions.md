...
需注意的是，在构造元组实例时，运行环境支持为成员添加名称。比如在调用函数时，可以按参数名称传参，或者混合按位置和按名称传参。但成员名称仅仅用于构造元组实例，因为这是运行环境内部所需的。XiaoXuan 没提供按元组成员名称访问成员的方法（即名称只是成员的一个附加的数据）。
...
元组常用的只有构造和读取两种操作，一般很少需要修改元组的数据，不过 XiaoXuan 还是提供了添加新成员到一个元组的头部的方法，用于构造一个新元组。语法是使用一对圆括号和 "..." 符号（三个点号）。
