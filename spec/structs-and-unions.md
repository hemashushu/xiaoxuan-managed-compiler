...
结构体的定义也可以写成一行的形式，只需把所有成员用括号包围起来，然后使用逗号分隔各个成员即可，语法如下。
...
运行环境会自动为每一个结构体创建一个同名的函数，即默认构造函数，函数的参数为全部成员。使用构造函数即可创建结构体的实例。

示例：

```js
让 u1 = 用户(100, "张三", true)
让 u2 = 用户(101, "李四", false)
```

```js
let u1 = User(100, "foo", true)
let u2 = User(101, "bar", false)
```
...
```js
让 u1 = 用户(id=100, name="张三", checked=true)
```
...
运行环境会自动为结构体创建一个默认构造函数，函数的参数为全部成员，返回值为该结构体。根据函数的重载原则，我们无法添加一个签名跟默认构造函数一样的函数，但可以添加一个或多个仅参数不同的函数，这些函数则为自定义构造函数。

示例：

```js
函数 用户(整数 id, 字符串 name) = 用户(id, name, true)
函数 用户(整数 id) = 用户(id, "张三")
```

```js
function User(Int id, String name) = User(id, name, true)
function User(Int id) = User(id, "张三")
```
...
元组（tuple）通常用于函数的返回值，当函数需要返回的数值不止一个时可以使用元组。一个元组可以视为一个匿名成员的结构体，即它由固定数量、顺序、数据类型的一个或多个成员组成。
...
除了模式匹配和模式解构，还可以通过运行环境自动为元组生成的成员名称来访问，成员名称的分别为 "_1"，"_2" 如此类推，即一个下划线加上成员的次序，次序从数字 1 开始（而不是从 0 开始）。
...
需注意的是，在构造元组实例时，运行环境是支持为值命名的。比如在调用函数时，可以按参数名称传参，或者混合按位置和按名称传参。但这仅仅用于构造元组实例，因为这是运行环境内部所需的。XiaoXuan 没提供按成员名称访问元组成员的方法。
...
元组常用的只有构造和读取两种操作，一般很少需要修改元组的数据，不过 XiaoXuan 提供了添加新成员到一个元组的头部的方法，用于构造一个新元组。语法是使用一对圆括号和 "..." 符号（三个点号）。
