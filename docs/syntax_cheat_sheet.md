...
- 上面的三个点 `...` 表示任意表达式；（`let` 关键字后面的子表达式除外，赋值表达式的 `左手边值` 只能是 `标识符`、`元组`、`列表`、`映射表` 以及字面量）
- 上面的花括号属于固定语法的一部分，比如关键字 `do` 后面必须带上一对花括号，而不能省略。
- 关键字 `if`、`then`、`else` 后面的表达式可以是 `隠式 do 表达式`，即一对花括号包围起来的表达式块，比如 `then do {...}` 可以写成 `then {...}`；
