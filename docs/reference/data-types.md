...
* `整数16`（`Int16`）
* `整数8`（`Int8`）
* `整数`（`Int`），为 `整数64` 的别名。<!-- 应该是 int32 -->

<!-- 考虑基本数据类型的名称为小写，即使用 `int32` 而不是 `Int32` -->
...
注意不支持类似 JavaScript 的 `\uHHHH` 表示方法。<!-- 这点尚未确定，如果考虑编程习惯最好保留跟 JS 行为一致 -->
...
<!-- 废弃
跟一般 C 的字符串不同，XiaoXuan Lang 的字符串最末尾不包含 `\0` 字符。
也就是说 `字符串` 类型实际上是对 `<字节>` 做了一层薄薄的包装（之所以需要包装，是因为在诸如 first(), each 等运算时，必须以 Unicode 码点作为单位）。
-->
...
注意分行符下一行的所有前缀空格符（包括制表符、换行符等）会被忽略，直到第一个非空格字符。注意这跟 JavaScript 不完全相同，JavaScript 里分行符的下一行字符串的前面空格会被包含进来。<!-- 这点尚未确定，如果考虑编程习惯最好保留跟 JS 行为一致 -->
...
<!-- 考虑使用 {{...}} -->
<!-- 暂时不考虑类似 JavaScript 的 ${...} 表达式支持，使用 join 宏即可  -- >

其中 `{...}` 为模板字符串占位符，占位符里面可以是其他数据的字面量，可以是一个上下文存在的变量，<!-- ::TODO:: 也可以是一个表达式，甚至可以是一段有返回值的语句 只要返回一个字符串即可。-->
...
##### 自动截除前缀空白 <!-- 考虑去除此特性，提供截除前缀空白的函数即可 -->
...
<!-- 以下特性考虑取消，使用 join {} 结构即可 --> 可以在模板字符串后面加入一个值，用于向模板处理函数传入数据，这种情况下 XiaoXuan 将不会解析模板字符串的内容，而是完全由模板字符串自己处理，比如 `handlebars` 函数：
...
::TODO
可以考虑 数字+r 的表示方法，类似：2r / 3，与复数的表示方法一致（如 2 + 3i） <!-- 废弃 -->
